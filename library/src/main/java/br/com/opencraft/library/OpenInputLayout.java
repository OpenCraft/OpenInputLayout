...
public class OpenInputLayout extends LinearLayout implements WithHint {
    ...
    private OpenCheckableImageButton mPasswordToggleView;
    ...
    private OpenCollapsingTextHelper mCollapsingTextHelper;
    ...
    //Defines whether the hint will move up or down
    private boolean mMoveLabelUp = true;
    private float mLabelSpacingExtra = 0f;

    public OpenInputLayout(Context context) {
        this(context, null);
    }

    public OpenInputLayout(Context context, AttributeSet attrs) {
...
//        ThemeUtils.checkAppCompatTheme(context);
        ...
        mCollapsingTextHelper.setTextSizeInterpolator(OpenAnimationUtils.FAST_OUT_SLOW_IN_INTERPOLATOR);
        mCollapsingTextHelper.setPositionInterpolator(new AccelerateInterpolator());
        mCollapsingTextHelper.setCollapsedTextGravity(Gravity.TOP | GravityCompat.START);

        final TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs,
                R.styleable.OpenInputLayout,
                defStyleAttr, android.support.design.R.style.Widget_Design_TextInputLayout);

        mHintEnabled = a.getBoolean(R.styleable.OpenInputLayout_hintEnabled, true);
        setHint(a.getText(R.styleable.OpenInputLayout_android_hint));
        mHintAnimationEnabled = a.getBoolean(R.styleable.OpenInputLayout_hintAnimationEnabled, true);

        if (a.hasValue(R.styleable.OpenInputLayout_android_textColorHint)) {
            mDefaultTextColor = mFocusedTextColor = a.getColorStateList(R.styleable.OpenInputLayout_android_textColorHint);
        }

        final int hintAppearance = a.getResourceId(R.styleable.OpenInputLayout_hintTextAppearance, -1);
        if (hintAppearance != -1) {
            setHintTextAppearance(a.getResourceId(R.styleable.OpenInputLayout_hintTextAppearance, 0));
        }

        mErrorTextAppearance = a.getResourceId(R.styleable.OpenInputLayout_errorTextAppearance, 0);
        final boolean errorEnabled = a.getBoolean(R.styleable.OpenInputLayout_errorEnabled, false);

        final boolean counterEnabled = a.getBoolean(R.styleable.OpenInputLayout_counterEnabled, false);
        setCounterMaxLength(a.getInt(R.styleable.OpenInputLayout_counterMaxLength, INVALID_MAX_LENGTH));
        mCounterTextAppearance = a.getResourceId(R.styleable.OpenInputLayout_counterTextAppearance, 0);
        mCounterOverflowTextAppearance = a.getResourceId(R.styleable.OpenInputLayout_counterOverflowTextAppearance, 0);
        mPasswordToggleEnabled = a.getBoolean(R.styleable.OpenInputLayout_passwordToggleEnabled, false);
        mPasswordToggleDrawable = a.getDrawable(R.styleable.OpenInputLayout_passwordToggleDrawable);
//        mPasswordToggleContentDesc = a.getText(R.styleable.OpenInputLayout_android_passwordToggleContentDescription);
        if (a.hasValue(R.styleable.OpenInputLayout_moveLabelUp)) {
            mMoveLabelUp = a.getBoolean(R.styleable.OpenInputLayout_moveLabelUp, true);
        }
        if (a.hasValue(R.styleable.OpenInputLayout_passwordToggleTint)) {
            mHasPasswordToggleTintList = true;
            mPasswordToggleTintList = a.getColorStateList(R.styleable.OpenInputLayout_passwordToggleTint);
        }

        mLabelSpacingExtra = a.getDimension(R.styleable.OpenInputLayout_labelSpacingExtra, 0f);
//        if (a.hasValue(R.styleable.OpenInputLayout_passwordToggleTintMode)) {
//            mHasPasswordToggleTintMode = true;
//            mPasswordToggleTintMode = ViewUtils.parseTintMode(
//                    a.getInt(android.support.design.R.styleable.OpenInputLayout_passwordToggleTintMode, -1), null);
//        }
...
//        ViewCompat.setAccessibilityDelegate(this, new TextInputLayout.TextInputAccessibilityDelegate());
    ...
    private void updateInputLayoutMargins() {
        ...
        final LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) mInputFrame.getLayoutParams();
        final int newMargin;
        ...
        if (mHintEnabled) {
            ...
            newMargin = mMoveLabelUp ? (int) -mTmpPaint.ascent() : (int) (mTmpPaint.descent() + (-mTmpPaint.ascent()));
        } else {
            newMargin = 0;
        }
        ...
        if (mMoveLabelUp) {
            if (newMargin != lp.topMargin) {
                lp.topMargin = newMargin + (int) mLabelSpacingExtra;
                mInputFrame.requestLayout();
            }
        } else {
            if (newMargin != lp.bottomMargin) {
                lp.bottomMargin = newMargin + (int) mLabelSpacingExtra;
                mInputFrame.requestLayout();
            }
        }
    }
     ...
     *
     * @attr ref android.support.design.R.styleable#OpenInputLayout_android_hint
     ...
     *
     * @attr ref android.support.design.R.styleable#OpenInputLayout_hintEnabled
     ...
     *
     * @attr ref android.support.design.R.styleable#OpenInputLayout_hintTextAppearance
     */
     ...
     *
     * @attr ref android.support.design.R.styleable#OpenInputLayout_errorEnabled
     */
     ...
     *
     * @attr ref android.support.design.R.styleable#OpenInputLayout_errorTextAppearance
     */
...
//        }
    ...
    static class SavedState extends AbsSavedState {
        ...
        public static final Creator<OpenInputLayout.SavedState> CREATOR = new ClassLoaderCreator<OpenInputLayout.SavedState>() {
            @Override
            public OpenInputLayout.SavedState createFromParcel(Parcel in, ClassLoader loader) {
                return new OpenInputLayout.SavedState(in, loader);
            }

            @Override
            public OpenInputLayout.SavedState createFromParcel(Parcel in) {
                return new OpenInputLayout.SavedState(in, null);
            }

            @Override
            public OpenInputLayout.SavedState[] newArray(int size) {
                return new OpenInputLayout.SavedState[size];
            }
        };
    }
    ...
    public Parcelable onSaveInstanceState() {
        ...
        OpenInputLayout.SavedState ss = new OpenInputLayout.SavedState(superState);
        if (mErrorShown) {
            ss.error = getError();
        }
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        if (!(state instanceof OpenInputLayout.SavedState)) {
            super.onRestoreInstanceState(state);
            return;
        }
        OpenInputLayout.SavedState ss = (OpenInputLayout.SavedState) state;
     ...
     * @attr ref android.support.design.R.styleable#OpenInputLayout_passwordToggleDrawable
     */
     ...
     * @attr ref android.support.design.R.styleable#OpenInputLayout_passwordToggleContentDescription
     */
     ...
     * @attr ref android.support.design.R.styleable#OpenInputLayout_passwordToggleEnabled
     */
     ...
     * @attr ref android.support.design.R.styleable#OpenInputLayout_passwordToggleTint
     */
     ...
     * @attr ref android.support.design.R.styleable#OpenInputLayout_passwordToggleTintMode
     */
    ...
    void animateToExpansionFraction(final float target) {
        ...
        if (mAnimator == null) {
            ...
            mAnimator.setInterpolator(OpenAnimationUtils.LINEAR_INTERPOLATOR);
